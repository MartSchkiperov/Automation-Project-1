...
describe('Section 1: Functional tests', () => {
    ...
    it('User can use only same both first and validation passwords', ()=>{
        ...
        cy.get('#confirm').clear().type('MyPass123')
        cy.get('h2').contains('Password').click()
    ...
    it('User must fill mandatory email field', ()=>{
        // Add test steps for filling in only mandatory fields
        cy.get('#username').type('Something')
        ...
        cy.get('#email').type('e@e.ee')
        cy.get('h2').contains('Password').click()
        // Filling e-mail the submit button is now enabled, it is possible to click it
        // and success message is given
        cy.get('.submit_button').should('be.enabled')
        cy.get('.submit_button').click()
        cy.get('#success_message').should('be.visible')
    })
...
describe('Section 2: Visual tests', () => {
    ...
    it('Check navigation of registration form 3', () => {
        cy.get('nav').children().should('have.length', 2)
